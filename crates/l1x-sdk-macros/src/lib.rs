...
use quote::quote;
use syn::{ItemImpl, Signature, Visibility};
...
///
/// The generated wrapper reads method arguments [`l1x_sdk::input`], deserializes them, and calls
/// the original method. When the original method returns, the wrapper serializes the returned value
/// and writes the serialized value with `l1x_sdk::output`
///
/// # Example
/// ```
...
pub fn contract(_attr: TokenStream, item: TokenStream) -> TokenStream {
	if let Ok(input) = syn::parse::<ItemImpl>(item) {
		...
		TokenStream::from(
			syn::Error::new(Span::call_site(), "#[contract] can only be used on impl sections.")
				.to_compile_error(),
		)
	}
}
