...
pub fn call_contract(call: &ContractCall) -> Option<Vec<u8>> {
	let call = call.try_to_vec().expect("Can't serialize the function arguments");
...
mod tests {
	...
	fn test_contract_owner_address_and_caller_address() {
		...
		assert_eq!(contract_owner_address(), Address::test_create_address(&mock_owner_address));

		// Test caller_address
		assert_eq!(caller_address(), Address::test_create_address(&mock_caller_address));
