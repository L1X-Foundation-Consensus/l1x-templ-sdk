...
mod tests {
	...
	fn test_set_persistence() {
		...
		assert!(stored_value_bytes.is_some(), "Expected the key to be set in storage");
	...
	fn test_update_persistence() {
		...
		assert_eq!(stored_value, TestValue(20), "Expected the value to be updated in storage");
	}
	...
	fn test_remove_persistence() {
		...
		assert!(stored_value_bytes.is_none(), "Expected the key to be removed from storage");
	}
