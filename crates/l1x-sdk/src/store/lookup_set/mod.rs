...
mod tests {
	...
	fn test_insert_duplicate_values() {
		...
		// Try to insert the same value again. This time it should return false because it is a
		// duplicate.
		assert!(!set.insert(TestValue(1)));
	}
	...
	fn test_insert_persistence() {
		...
		assert!(stored_value.is_some(), "Expected the value to be set in storage");
	}
