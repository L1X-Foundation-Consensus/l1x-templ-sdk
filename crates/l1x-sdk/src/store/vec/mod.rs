	...
	T: BorshSerialize + BorshDeserialize,
{
	...
	pub fn push(&mut self, element: T) {
		...
		self.len = self.len.checked_add(1).unwrap_or_else(|| crate::panic(ERR_INDEX_OUT_OF_BOUNDS));
	...
	T: BorshSerialize + BorshDeserialize + Copy,
{
	...
	///
	/// This does not preserve ordering, but is O(1). If you need to preserve the element order, use
	/// remove instead.
	///
	/// # Panics
	///
